...
public interface ShortcutConfigurable {
	...
	static Object getValue(SpelExpressionParser parser, BeanFactory beanFactory,
		...
		if (rawValue != null && rawValue.startsWith("#{") && entryValue.endsWith("}")) {
			// assume it's spel
			SentinelEvaluationContext context = new SentinelEvaluationContext(
					beanFactory);
	...
	class SentinelEvaluationContext implements EvaluationContext {

		private final BeanFactoryResolver beanFactoryResolver;

		private final SimpleEvaluationContext delegate;

		public SentinelEvaluationContext(BeanFactory beanFactory) {
			this.beanFactoryResolver = new BeanFactoryResolver(beanFactory);
			delegate = SimpleEvaluationContext
					.forPropertyAccessors(new RestrictivePropertyAccessor())
					.withMethodResolvers(
							(context, targetObject, name, argumentTypes) -> null)
					.build();
		}
